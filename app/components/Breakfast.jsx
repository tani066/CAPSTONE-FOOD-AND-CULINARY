...
export default function BreakfastRecipes() {
  ...
  const apiKey = process.env.NEXT_PUBLIC_SPOONACULAR_API_KEY; // Replace with your API key
  ...
  useEffect(() => {
    const fetchBreakfastRecipes = async () => {
      const url = `https://api.spoonacular.com/recipes/complexSearch?apiKey=${apiKey}&number=100&type=breakfast`;
      ...
      try {
        ...
        console.error('Failed to fetch breakfast recipes:', err);
    ...
    <div className="p-4 min-h-screen">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-12">
          <h2 className="text-4xl font-bold mb-3 text-orange-600 font-serif">🍳 Morning Delights</h2>
          <p className="text-gray-600 text-lg italic mb-8">Start your day with these delicious breakfast recipes</p>
          <div className="relative max-w-xl mx-auto">
            <input
              type="text"
              placeholder="Search for breakfast recipes..."
              value={searchQuery}
            ...
            >
              ...
              <div className="p-4">
                <button
                  ...
                  className="w-full bg-orange-500 hover:bg-orange-600 text-white font-medium py-3 px-6 rounded-lg transition-all duration-300 transform hover:shadow-lg hover:-translate-y-0.5 active:translate-y-0 active:shadow-md"
                >
                  View Recipe
                </button>
              </div>
      ...
      {selectedRecipe && (
        <div className="fixed inset-0 z-50 bg-black/90 backdrop-blur-sm flex items-center justify-center p-4 transition-opacity duration-300">
          <div className="bg-white/95 backdrop-blur-sm rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto relative border border-orange-100/20">
            <button
              className="absolute top-4 right-4 bg-orange-500 hover:bg-orange-600 text-white rounded-full w-8 h-8 flex items-center justify-center transition-colors duration-300"
            ...
            {loadingDetails ? (
              ...
              <>
                <div className="p-8">
              <h2 className="text-4xl font-bold mb-4 text-orange-600 font-serif leading-tight">{selectedRecipe.title}</h2>
              <div className="relative h-80 w-full rounded-xl overflow-hidden mb-6">
                ...
                {selectedRecipe.summary && (
                  <div className="prose max-w-none text-gray-700 bg-orange-50/50 backdrop-blur-sm p-6 rounded-xl mb-8 shadow-sm border border-orange-100/20"
                ...
                {selectedRecipe.extendedIngredients && (
                  <div className="mt-8 bg-white/80 backdrop-blur-sm p-8 rounded-xl shadow-md border border-orange-100/30 hover:bg-white/90 transition-colors duration-300">
                  <h3 className="font-bold text-xl mb-4 text-orange-600 flex items-center gap-2">
                ...
                {selectedRecipe.analyzedInstructions?.[0]?.steps && (
                  <div className="mt-8 bg-white/80 backdrop-blur-sm p-8 rounded-xl shadow-md border border-orange-100/30 hover:bg-white/90 transition-colors duration-300">
                    <h3 className="font-bold text-xl mb-6 text-orange-600 flex items-center gap-2">
                      <span className="bg-orange-100 p-2 rounded-full">📋</span>
                      Instructions
                    </h3>
                    <ol className="space-y-6">
                      {selectedRecipe.analyzedInstructions[0].steps.map((step) => (
                        <li key={step.number} className="flex gap-4 items-start">
                          <span className="bg-orange-500 text-white font-bold rounded-full w-8 h-8 flex items-center justify-center flex-shrink-0 shadow-md">
                            {step.number}
                          </span>
                ...
                {selectedRecipe.sourceUrl && (
                  <div className="mt-8 pt-6 border-t border-orange-100">
                    <p className="text-sm text-gray-500 italic">
                      Source: <a href={selectedRecipe.sourceUrl} target="_blank" rel="noopener noreferrer" className="text-orange-500 hover:underline transition-colors duration-300">View original recipe</a>
                    </p>
                  </div>
                )}
              </div>
